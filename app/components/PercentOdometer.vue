<template>
  <TransitionGroup
    ...
    class="font-mono relative inline-flex items-baseline [clip-path:inset(0)] align-baseline tabular-nums whitespace-nowrap tracking-normal font-features-['tnum'_1] [font-kerning:none]"
...
const props = withDefaults(defineProps<{ value: number; suffix?: string; decimals?: number }>(), { suffix: "%", decimals: 0 });
...
const { ariaLabel, columns } = useOdometer(() => props.value, { suffix: props.suffix, decimals: props.decimals });
</script>
